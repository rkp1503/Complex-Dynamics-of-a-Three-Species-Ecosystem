...
function my_phase_portraits(solutions, xaxis, yaxis, zaxis)
    ...
    fig_a = my_3D_phase_portrait(solutions, "3D Phase Portrait", xaxis, yaxis, zaxis)
    fig_b = my_2D_phase_portrait(solution_x, solution_y, "Species X and Y", xaxis, yaxis)
    fig_c = my_2D_phase_portrait(solution_x, solution_z, "Species X and Z", xaxis, zaxis)
    fig_d = my_2D_phase_portrait(solution_y, solution_z, "Species Y and Z", yaxis, zaxis)
    l = @layout [a b ; c d]
...
function my_bifurcation_diagrams(solutions, data, variables, bifurcartion_parameter, legend_data)
    ...
    fig_a = my_plot(solutions, title, xaxis, yaxis, legend_data)
    fig_b = my_bifurcation_diagram(x_arr_s, x_arr_u, x, bifurcartion_parameter, "black")
    fig_c = my_bifurcation_diagram(y_arr_s, y_arr_u, y, bifurcartion_parameter, "red")
    fig_d = my_bifurcation_diagram(z_arr_s, z_arr_u, z, bifurcartion_parameter, "blue")
    l = @layout [a b ; c d]
    return plot(fig_a, fig_b, fig_c, fig_d, layout=l)
end;
