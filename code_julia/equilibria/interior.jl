...
function analytical_solution(param_vals)
    ...
    y = -1
    z = -1
    Y₀ = u₂^2*(r₃₁*u₁*(u₄-r₃₁)*(1-p)+r₂₁*u₂*(ϕ₂₁*(ϕ₁₃*(r₃₁-u₄)-r₃₁)^2+r₃₁^2))
    Y₁ = -u₂*(r₃₁*u₁*(2*(r₃₁-u₄)+u₃)*(1-p)^2+r₂₁*u₂*(-3*ϕ₂₁*ϕ₁₃^2*u₄^2-3*r₃₁^2*((1-ϕ₁₃)^2*ϕ₂₁+1)+6*r₃₁*ϕ₂₁*ϕ₁₃*u₄*(ϕ₁₃-1)+2*ϕ₁₃*ϕ₂₁*u₃*(ϕ₁₃*(u₄-r₃₁)+r₃₁))*(1-p)+r₂₁*r₃₁^2*u₂^2)
    Y₂ = r₃₁*u₁*(u₄-r₃₁-u₃)*(1-p)^3+r₂₁*u₂*(ϕ₂₁*ϕ₁₃^2*(3*r₃₁^2+2*r₃₁*(2*u₃-3*u₄)+u₃^2+3*u₄^2-4*u₃*u₄)-2*r₃₁*ϕ₂₁*ϕ₁₃*(3*(r₃₁-u₄)+2*u₃)+3*r₃₁^2*(ϕ₂₁+1))*(1-p)^2-3*r₂₁*r₃₁^2*u₂^2*(1-p)+2*r₂₁*r₃₁*ϕ₁₂*ϕ₂₁*u₂^3*(r₃₁-ϕ₁₃*(r₃₁-u₄))
    Y₃ = r₂₁*(1-p)*((ϕ₂₁*(ϕ₁₃*(r₃₁+u₃-u₄)-r₃₁)^2+r₃₁^2)*(1-p)^2-3*r₃₁^2*u₂*(1-p)-2*r₃₁*ϕ₁₂*ϕ₂₁*u₂^2*(ϕ₁₃*(3*(r₃₁-u₄)+u₃)-3*r₃₁))
    Y₄ = r₂₁*r₃₁*(-r₃₁*(1-p)^3-2*ϕ₁₂*ϕ₂₁*u₂*(ϕ₁₃*(3*(r₃₁-u₄)+2*u₃)-3*r₃₁)*(1-p)^2+r₃₁*ϕ₁₂^2*ϕ₂₁*u₂^2)
